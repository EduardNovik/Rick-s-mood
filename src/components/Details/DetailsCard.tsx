...
type handleExpandClickProp = () => void
...
const DetailsCard: FC = (): ReactElement => {
  const characterDetails = useSelector<RootState, CharacterResultsProp | []>(
    (state) => state.characterDetails.data
  );

  const [expanded, setExpanded] = useState<boolean>(false);

  const handleExpandClick: handleExpandClickProp = () => {
  ...
  return (
    <Box sx={{ display: "flex", justifyContent: "center", pt: "150px" }}>
      {Array.isArray(characterDetails) ? (
        <Typography>There is no information about this character</Typography>
      ) : (
        <>
          <Card
            sx={{
